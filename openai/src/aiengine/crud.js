...
export async function READ(prompt, input) {
    ...
    try {
        ...
        const database = client.db("thevectordatabase");
        const collection = database.collection("testVector");
        const queries = input;
        ...
        for (let i = 0; i < queries.length; i++) {
            const query = { patientName: queries[i] };
        ...
        let new_prompt =
            "Forget any previous prompt. Answer the following as a Nurse AI, be professional and get to the point concisely. Do NOT include symbols, code, orformulas in response. Only use the given context for your response:<<<" +
            prompt +
            `>>>, given the context: <<<${context.toString()}>>>. You are .`;
...
export async function UPDATE(input) {
    ...
    try {
        ...
        for (let i = 0; i < input.length; i++) {
            ...
            const filter = { patientName: outerKey };
            const updateDoc = {
                $set: {},
            };
